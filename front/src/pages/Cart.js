...
import { getUserCart, removeProductFromCart, updateProductFromCart } from "../redux/slices/auth";

const Cart = () => {
  const dispatch = useDispatch();
  const userCartState = useSelector((state) => state?.auth?.cartIt?.items)
  ...
  if(productUpdateDetail !== null) {
    dispatch(updateProductFromCart({cartItemId:productUpdateDetail?.cartItemId,quantity:productUpdateDetail?.quantity}))
    setTimeout(() => {
      dispatch(getUserCart())
    }, 200)
  }
  ...
  const updateACartProduct = (productUpdateDetail) => {
    // dispatch(updateProductFromCart(id,newQuantity))
    // setTimeout(() => {
    //   dispatch(getUserCart())
    // }, 200)
  }
